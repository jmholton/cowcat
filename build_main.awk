...
#
#
#	(re)build a polypeptide chain with given phi-psi-chi angles
#
#
# commands are formatted like this:
# BUILD RES Chain Num phi psi chi1 chi2 chi3 ...
#
# EG:
# BUILD ARG A 51   -64 -40   -67.6 176.9 ?
# BUILD LEU - 66   ? ?   - t
#
#
BEGIN{
...
/^ATOM/{
    ...
    X[atom,chain,resnum]      = substr($0, 31, 8)+0
    Y[atom,chain,resnum]      = substr($0, 39, 8)+0
    Z[atom,chain,resnum]      = substr($0, 47, 8)+0
    Occ[atom,chain,resnum]    = substr($0, 55, 6)+0
    Bfac[atom,chain,resnum]   = substr($0, 61, 6)+0
}



END{
    # now run down full atom list
    for(i=1;i<=atoms;++i)
    {
	if(BUILD[Chain[atom],ResNUM[atom]+1])
	{
	    # request to rebuild NEXT residue
	}
	
	if(BUILD[Chain[atom],ResNUM[atom]-1])
	{
	    # request to rebuild LAST residue
	}
	
	if(BUILD[Chain[atom],ResNUM[atom]])
	{
	    # request to rebuild THIS residue
	}	
    }
    # now review the building commands
    ...
    if(build=="C2N")
    {
	# now print out saved, old atoms
	for(i=1;i<=old_atoms;++i) print OLD_ATOMS[i]
    }

    if(! atoms_printed) exit
    
    if(build=="N2C") print_atom(N,"OT",Restyp,Resnum);
    print "END"
}
